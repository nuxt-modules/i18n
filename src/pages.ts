...
export async function setupPages({ localeCodes, options, isSSR }: I18nNuxtContext, nuxt: Nuxt) {
  ...
  const typedRouter = await setupExperimentalTypedRoutes(nuxt)
  ...
  extendPages(async pages => {
    ...
    if (useExperimentalTypedPages) {
      ...
      const context = typedRouter.createTypedRouterContext(pages, dtsFile)
 ...
 */
async function setupExperimentalTypedRoutes(nuxt: Nuxt) {
  const useExperimentalTypedPages = nuxt.options.experimental.typedPages
  const dtsFile = resolve(nuxt.options.buildDir, declarationFile)

  /**
   * Set to contents of dts every time we read {@link declarationFile}
   * required since we cannot read the file while writing to it during prepare step
   */
   ...
   */
  function createTypedRouterContext(pages: NuxtPage[], dtsFile: string) {
    ...
    context.scanPages = async function (watchers = false) {
      ...
      dtsContent = await readFile(dtsFile, 'utf-8')
...
export {}`
    ...
    const context = createTypedRouterContext(nuxt.apps.default?.pages ?? [], dtsFile)

    await mkdir(dirname(dtsFile), { recursive: true })
    await context.scanPages(false)

    dtsContent = await readFile(resolve(nuxt.options.buildDir, declarationFile), 'utf-8')

    if (nuxt.options._prepare || !nuxt.options.dev) {
      // TODO: could we generate this from context instead?
      addTemplate({
        filename: 'types/typed-router-i18n.d.ts',
        getContents: () => dtsContent.replace(routeNamedMapTypeRE, 'RouteNamedMapI18n')
      })
    }

    nuxt.hook('app:templatesGenerated', async (_app, _templates, options) => {
      // @ts-ignore
      if (!options?.filter || options.filter({ filename: 'routes.mjs' })) {
        await context.scanPages(false)
      }
    })
  }
