...
export async function setupPages({ localeCodes, options, isSSR }: I18nNuxtContext, nuxt: Nuxt) {
  ...
  function createI18nTypedRouterContext(pages: NuxtPage[], dtsFile: string) {
...
export {}`
    ...
    const context = createI18nTypedRouterContext(nuxt.apps.default?.pages ?? [], dtsFile)
  ...
  extendPages(async pages => {
    ...
    if (useExperimentalTypedPages) {
      const dtsFile = resolve(nuxt.options.buildDir, declarationFile)
      const context = createI18nTypedRouterContext(pages, dtsFile)
