...
import { parseAcceptLanguage, runtimeDetectBrowserLanguage } from '../internal'
import { nuxtI18nInternalOptions, localeCodes as _localeCodes } from '#build/i18n.options.mjs'
...
export function useCookieLocale(
  options: { localeCodes: readonly string[] } = { localeCodes: _localeCodes }
): Ref<string> {
  ...
  if (!opts || !opts.useCookie) {
    return locale
  }

  let code: string | null = null
  if (process.client) {
    const cookie = useCookie<string>(opts.cookieKey!) as Ref<string>
    code = cookie.value
  } else if (process.server) {
    const cookie = useRequestHeaders(['cookie'])
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    code = (cookie as any)[opts.cookieKey!]
  }
