...
export async function extendMessages(nuxt: Nuxt, localeCodes: string[]): Promise<AdditionalMessages> {
  ...
  return normalizeMessages(additionalMessages, localeCodes)
}

async function normalizeMessages(additional: LocaleMessages<DefineLocaleMessage>[], localeCodes: string[]) {
  /**
   * merge additional messages into vueI18n messages
   */

  const targetLocaleCodes = [...localeCodes]
