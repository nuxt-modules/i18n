...
- `autoImportTranslationFunctions` (default: `false`) - Makes `$t`, `$d` and `$n` functions available globally, equivalent to `const { t: $t, d: $d, n: $n } = useI18n({ useScope: 'global' })` in setup.
