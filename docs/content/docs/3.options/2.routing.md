...
- `domains` (required when using [`differentDomains`](/docs/options/domain#differentdomains) while one or more of the domains having multiple of the same locales) - an array of [`domain`].
- `defaultForDomains` - an array of [`domain`] for which the locale should be the default locale when using [`domains`].
- `domainDefault` (required when using [`differentDomains`](/docs/options/domain#differentdomains) while one or more of the domains having multiple locales) - set `domainDefault` to `true` for each locale that should act as a default locale for the particular domain(s).
