...
If `customRoutes` option is disabled with `config`, the module will look for custom routes in the `pages` option. Refer to the [Routing](/docs/guide/routing-strategies) for usage.
...
The `dynamicRouteParams` is deprecated and will be removed in `v8.1`, use the [`useSetI18nParams`](/docs/api/composables#useseti18nparams) composable instead.
::
