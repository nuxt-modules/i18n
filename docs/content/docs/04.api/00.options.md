...
- Doesn't have to be the same array for every locale, a locale is only available on the domains listed here (or in `defaultForDomains`). On a domain that isn't listed, the locale is left out of `locales` and its routes return a 404. A locale with no `domains` at all is treated as available on every domain. See [Restricting locales to specific domains](/docs/guide/multi-domain-locales#restricting-locales-to-specific-domains).
