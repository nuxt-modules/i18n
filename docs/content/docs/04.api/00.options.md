...
- type: `number | undefined`{lang="ts-type"}
- default: `undefined`{lang="ts"}

Specifies the HTTP status code to use when redirecting from any URL except the root URL ('/'). Accepts a number. If not specified, the default status code used by navigateTo will be applied (typically 302).
```json
{
