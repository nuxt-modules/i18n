...
- type: `boolean`{lang="ts-type"}
- default: `true`{lang="ts"}
- Generates route types used in composables and configuration, this feature is enabled by default when Nuxt's `experimental.typedPages` is enabled.

::callout{icon="i-heroicons-exclamation-triangle" color="warning"}
This feature relies on [Nuxt's `experimental.typedPages`](https://nuxt.com/docs/guide/going-further/experimental-features#typedpages) and will not work if this is not enabled.
::
...
- type: `boolean`{lang="ts-type"}
- default: `false`{lang="ts"}
- Names the unprefixed route tree `<base>___default` instead of `<base>___<defaultLocale>`, gives the build's own default locale a prefixed tree alongside it, and resolves the deployment's default locale to that locale-agnostic name at runtime. The unprefixed tree can then be addressed without knowing which locale the build defaulted to, so one build can serve several deployments that each set `defaultLocale` through runtime config (e.g. `NUXT_PUBLIC_I18N_DEFAULT_LOCALE`). Requires the `prefix_except_default` strategy; ignored under `differentDomains` and `multiDomainLocales`, where the unprefixed locale is already resolved per host. Pairs with `compactRoutes`, which collapses the prefixed locales into a single regex route.

::callout{icon="i-heroicons-light-bulb"}
Locale detection is unaffected: `<base>___default` yields a non-locale `"default"`, which fails the supported-locale check, so detection falls back to the deployment's configured default.
::
...
- type: `boolean`{lang="ts-type"}
- default: `false`{lang="ts"}
- Prerender the hashed `messages.json` files into `.output/public/` at build time so lazy-loaded messages are served as static assets instead of from the dynamic Nitro route. When Nuxt's `app.cdnURL` is set the client fetches those files from the CDN. See [Prerendering messages as static files](#prerendering-messages-as-static-files).

::callout{icon="i-heroicons-exclamation-triangle" color="warning"}
Not every locale can be prerendered. A locale whose messages are only produced by running its loader would be frozen at build time, and one holding [message functions](/docs/guide/lazy-load-translations) would have them stripped, so both keep loading through their loaders and keep fetching the Nitro route rather than the CDN.
...
Recommended setup for blue/green deployments:

1. Run `nuxt build` — hashed `messages.json` files are written into `.output/public/_i18n/<hash>/<locale>/messages.json` along with the rest of your static assets.
2. Upload `.output/public/` to the same origin (e.g. S3) that serves your `_nuxt/*` chunks.
3. Point your CDN (e.g. CloudFront) at that origin so each color of a deploy serves its own immutable, content-hashed messages — no sticky sessions required.
