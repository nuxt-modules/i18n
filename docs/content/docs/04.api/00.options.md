...
- type: `boolean`{lang="ts-type"}
- default: `false`{lang="ts"}
- Resolves the deployment's default locale to the locale-agnostic `<base>___default` route name instead of `<base>___<defaultLocale>`, so the unprefixed route tree can be addressed without knowing which locale the build defaulted to. Requires the `prefix_except_default` strategy and a build step that names the unprefixed tree `___default`; ignored under `differentDomains` and `multiDomainLocales`. Intended for serving one build from several deployments that each set `defaultLocale` through runtime config.
