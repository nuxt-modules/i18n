...
We now use a default folder structure that is consistent with [directory structure changes in Nuxt 4](https://nuxt.com/docs/getting-started/upgrade#new-directory-structure). I18n files are resolved from `<rootDir>/i18n` and `langDir` has a default value of `locales`.
...
Here is an example of a project structure after this change with `langDir` using its default value of `'locales'`:
