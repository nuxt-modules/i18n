...
The options parameter for `useLocaleHead()`{lang="ts"} and `$localeHead()`{lang="ts"} as changed in shape, having less verbose property names, as well as enabling the options by default.
...
We have added a `lang` property to the options parameter of `useLocaleHead()`{lang="ts"} and `$localeHead()`{lang="ts"}, originally this was not configurable on its own, see [`useLocaleHead()`{lang="ts"}](/docs/v9/api#useLocaleHead) for details on its usage.

## Nuxt context functions

In v8 both the types and name of the injected context functions (such as `$localePath()`{lang="ts"}, `$switchLocalePath()`{lang="ts"} and [more](/docs/v9/api/nuxt)) did not work as intended. You may have found that these functions worked when using them without prefix (`$`) even when not assigning these functions from a composable. 
...
Setting dynamic route parameters by setting the `nuxtI18n` property with `definePageMeta()`{lang="ts"} has been fully removed in favor of the [`useSetI18nParams()`{lang="ts"} composable](/docs/api#useseti18nparams), this feature is enabled by default which means the `dynamicRouteParams` option is no longer necessary.
