...
If a route has custom paths from multiple sources, the `definePageMeta()`{lang="ts"} value takes precedence and a warning is logged during build. This makes migration incremental: you can move custom paths from the `pages` option (or the deprecated macro) into page components one route at a time without switching modes.
...
The `i18n` property in route meta is the canonical source of custom paths at build time, `definePageMeta()`{lang="ts"} and the other sources are normalized into it before routes are localized. This means projects and modules can set custom paths on routes they add or mutate in the [`pages:extend`](https://nuxt.com/docs/api/advanced/hooks#nuxt-hooks-build-time) hook:
  ...
  hooks: {
    'pages:extend'(pages) {
      pages.push({
        ...
        meta: {
          i18n: {
            paths: { fr: '/soupe' } // -> accessible at /soup and /fr/soupe
          }
        }
      })
    }
...
In addition to `paths`, the meta value accepts `locales` to restrict which locales the route is localized in, and `i18n: false`{lang="ts"} disables localization for the route entirely.

::callout{icon="i-heroicons-exclamation-triangle" color="warning"}
With Nuxt's default `experimental.scanPageMeta: 'after-resolve'`{lang="yml"}, routes are localized in the `pages:resolved` hook, so paths added in any `pages:extend` hook are picked up. With `scanPageMeta: true`{lang="yml"}, localization runs in `pages:extend` itself and only hooks registered earlier (e.g. by modules listed before `@nuxtjs/i18n`) are.
::

## `defineI18nRoute`

::callout{icon="i-heroicons-exclamation-triangle" color="warning" title="notice"}
This method is deprecated in favor of `definePageMeta()`{lang="ts"} described above, and will be removed in v11.
::
