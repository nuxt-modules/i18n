...
::callout{icon="i-heroicons-light-bulb"}
[Message functions](https://vue-i18n.intlify.dev/guide/advanced/function.html) cannot be sent to the browser as JSON, so a locale file containing them is loaded by the browser directly instead. That ships the locale in the client bundle, so prefer plain messages where you can. The build only detects message functions written literally in the locale file - not values a loader computes or fetches at runtime - so write them in the locale file itself. In development, message functions that escape detection are reported with a warning.
::
