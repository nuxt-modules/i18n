...
::callout{icon="i-heroicons-light-bulb"}
[Message functions](https://vue-i18n.intlify.dev/guide/advanced/function.html) can not be sent to the browser as JSON, so a locale file containing them is detected during the build and loaded by the browser directly instead. That ships the locale in the client bundle, so prefer plain messages where you can. Messages a loader fetches at runtime can not be checked this way - write message functions in the locale file itself.
::
