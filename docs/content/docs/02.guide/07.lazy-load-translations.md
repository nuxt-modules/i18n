...
::callout{icon="i-heroicons-light-bulb"}
[Message functions](https://vue-i18n.intlify.dev/guide/advanced/function.html) cannot be sent to the browser as JSON, so a locale file containing them is loaded from the file itself instead of from the messages endpoint. That ships the locale in the client bundle, so prefer plain messages where you can.
...
::callout{icon="i-heroicons-light-bulb"}
The server runs loader functions outside the Nuxt app, so they can only use APIs that exist there too - `$fetch()`{lang="ts"}, `useRuntimeConfig()`{lang="ts"}, the h3 utilities. A locale file calling a Nuxt app composable (`useNuxtApp()`{lang="ts"}, `useState()`{lang="ts"}, `useCookie()`{lang="ts"}, ...) is loaded inside the Nuxt app instead, on both server and client, and is not served from the messages endpoint.
...
::callout{icon="i-heroicons-light-bulb"}
`cache` also decides where a locale's messages come from. A file that returns messages via a function and is not cacheable cannot be resolved at build time, so when there is no server to run it per request - while prerendering, or on a statically hosted build - that locale keeps its loader in the client bundle instead of being served from the messages endpoint. Setting `cache: true` on such a file opts it back into endpoint delivery.
::
